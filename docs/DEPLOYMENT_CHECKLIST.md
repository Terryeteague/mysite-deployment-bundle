# ✅ Deployment Checklist — Machine Design AI
...
- [ ] All internal links (`<a href>`) resolve correctly — no 404s
- [ ] Favicon (`/assets/favicon.svg`) is present
- [ ] `<title>` tags are unique and descriptive on every page
- [ ] `<meta name="description">` is filled in on every page (50–160 chars)
...
- [ ] Dependencies audited: `npm audit` or equivalent shows 0 high/critical issues
...
- [ ] Page `lang` attribute is set on `<html>`
...
- [ ] Server timezone is set to UTC (`timedatectl set-timezone UTC`)
...
*Last updated: June 2026 | Machine Design AI Assistant*
